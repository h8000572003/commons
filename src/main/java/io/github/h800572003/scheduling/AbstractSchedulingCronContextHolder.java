...
public abstract class AbstractSchedulingCronContextHolder
    ...
    protected static final byte[] UP_LOCK = new byte[]{};
    private int progress = -1;

    AbstractSchedulingCronContextHolder(IScheduingKey scheduingKey, TaskScheduler taskScheduler,
    ...
    public void runOnce() {
        ...
        new Thread(() -> AbstractSchedulingCronContextHolder.this.run()).start();
    ...
    private void shutdownAction(boolean mayInterruptIfRunning) {
        this.setUp(false);
        log.info("call {}服務中斷", this.scheduingKey.getCode());
        if (this.scheduledFuture != null && !this.scheduledFuture.isCancelled()) {
            log.info("call cancel code:{} ", this.scheduingKey.getCode());
            final boolean cancel = this.scheduledFuture.cancel(mayInterruptIfRunning);
